...
Uses quiet NetworkX Louvain by default. Set GRAPH_USE_LEIDEN=1 to opt into
graspologic Leiden when that optional dependency is installed.
"""
...
def _partition(G: nx.Graph) -> dict[str, int]:
    ...
    Uses Louvain (built into networkx) by default. Tries Leiden (graspologic)
    only when GRAPH_USE_LEIDEN=1 is set.
    ...
    """
    if os.getenv("GRAPH_USE_LEIDEN") == "1":
        try:
...
def cluster(G: nx.Graph) -> dict[int, list[str]]:
    ...
    a second community pass on the subgraph.
