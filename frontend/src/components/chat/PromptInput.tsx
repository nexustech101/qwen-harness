...
import { Send, ChevronUp, Check, Cpu } from "lucide-react"
...
export function PromptInput({ centered }: { centered?: boolean }) {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      if (activeSessionId) {
        ...
        setTimeout(() => {
          addUserMessage(text)
          api.sessions.sendPrompt(sess.id, body).catch((err) => {
  ...
  }, [prompt, activeSessionId, isRunning, createSession, sendPrompt, addUserMessage, clearChat, selectedModel, setActiveSession])
  ...
  return (
    <div className={cn(centered ? "w-full" : "bg-background p-3")}>
      <div className={cn(centered ? "w-full" : "mx-auto max-w-2xl")}>
        <div className="relative rounded-2xl border border-border/30 bg-card focus-within:ring-1 focus-within:ring-ring/40 transition-all">
                      ...
                      >
                        ...
                        <div className="flex-1 text-left min-w-0">
                          ...
                          <div className="text-xs text-muted-foreground">
                            {[m.parameter_size, m.family, m.quantization_level].filter(Boolean).join(" Â· ") || m.name}
                          </div>
                        </div>
